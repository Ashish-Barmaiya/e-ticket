...
const extractAadhaarXml = async (req, res) => {
  try {
    ...
    console.log(extractedDataFromXmlFile);
...
const verifyUserMobileAndSendOtp = async (req, res) => {
  try {
    ...
    const existingEkycWithPhoneNumber = await prisma.user.findFirst({
      where: { aadhaarPhoneNumber: aadhaarPhoneNumber },
    });
...
const userVerifyOtp = async (req, res) => {
  try {
    ...
    const encryptedAadhaarPhoneNumber = await encryptData(
      userAadhaarPhoneNumber,
    );
    ...
    await prisma.$transaction(async (prisma) => {
      ...
      console.log(
        "User eKyc successful. Unique User Identity is: ",
        uniqueUserIdentity,
      );
